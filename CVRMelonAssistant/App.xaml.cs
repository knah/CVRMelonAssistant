...
namespace CVRMelonAssistant
{
    ...
    {
        public static string ChilloutInstallDirectory;
        public static string ChilloutInstallType;
        ...
        private async void Application_Startup(object sender, StartupEventArgs e)
        {
            ...
            if (CVRMelonAssistant.Properties.Settings.Default.UpgradeRequired)
            {
                CVRMelonAssistant.Properties.Settings.Default.Upgrade();
                CVRMelonAssistant.Properties.Settings.Default.UpgradeRequired = false;
                CVRMelonAssistant.Properties.Settings.Default.Save();
            }

            Pages.Options options = Pages.Options.Instance;
            options.InstallDirectory =
                ChilloutInstallDirectory = Utils.GetInstallDir();

            Languages.LoadLanguages();

            while (string.IsNullOrEmpty(ChilloutInstallDirectory))
            {
                ...
                {
                    ChilloutInstallDirectory = Utils.GetManualDir();
                }
                else
                {
                    Environment.Exit(0);
            ...
            options.InstallType =
                ChilloutInstallType = CVRMelonAssistant.Properties.Settings.Default.StoreType;
            options.CloseWindowOnFinish =
                CloseWindowOnFinish = CVRMelonAssistant.Properties.Settings.Default.CloseWindowOnFinish;
                ...
                switch (args[0])
                {
                    ...
                    case "--language":
                        ...
                        else
                        {
                            ...
                            {
                                CVRMelonAssistant.Properties.Settings.Default.LanguageCode = args[1];
                                CVRMelonAssistant.Properties.Settings.Default.Save();
